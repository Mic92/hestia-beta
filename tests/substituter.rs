...
async fn evicted_pack_negative_caches_narinfo() {
    timed(async {
        ...
        let other = store.add_fixture("neg-cache-other", 88);

        let fake = FakeGha::start().await;
        let http = reqwest::Client::new();
        let snapshot = push_paths(&fake, &http, &store, &[&fixture, &other]).await;
        let substituter = RunningSubstituter::start(&fake, &http, &store).await;
        ...
        // So does a NAR request straight from nix's narinfo cache for
        // another path in that pack, without claiming it for GC.
        let probe = RunningSubstituter::start(&fake, &http, &store).await;
        let text = probe.narinfo(&http, &other).await.text().await.unwrap();
        let url = &parse_narinfo(&text)["URL"];
        assert_eq!(substituter.get(&http, url).await.status(), 404);
        assert!(
            !substituter
                .access_log
                .snapshot()
                .contains(&path_hash_of(&other)),
            "known-evicted pack must not record an access"
        );
