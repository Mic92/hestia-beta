...
pub async fn upload_pack(backend: &Backend, pack: &chunker::Pack) -> Result<(), GhaError> {
    backend.put(&pack.cache_key(), pack.data.clone()).await
}
...
pub struct PipelineContext {
    ...
    /// Branch and system of this daemon's own root (`<branch>-<system>`).
    pub branch: String,
    pub system: String,
...
impl PipelineContext {
    /// This daemon's own root, e.g. `main-x86_64-linux`.
    pub fn root_key(&self) -> String {
        root_key(&self.branch, &self.system)
    }

    /// Run the write pipeline into this daemon's own root.
    ///
    ...
    pub async fn run(
        ...
        self.run_root(&self.root_key(), paths, accessed).await
    }

    /// Same, into `root`. Only the own root claims against (and refreshes)
    /// the served view; another system's root is loaded on its own so the
    /// claim names exactly what that root holds.
    pub async fn run_root(
        ...
        // The served view dates from when the job started and GC may have
        // run since; a head based on it would be stale to the next GC. List
        // again now (reusing what is already loaded) so the base is at most
        // one drain old.
        let served = self
            .publish
            .as_ref()
        ...
        let mut stored: Vec<(String, PathInfo)> = Vec::new();
        ...
        for (path, lookup) in lookups {
            ...
            if snapshot.contains(&info.path_hash()) {
                stored.push((path, info));
            } else {
                to_push.push((path, info));
            }
        }
        // A stored path is re-claimed by copying its entry, which needs its
        // segment's tree. With the tree gone the path is unservable as
        // stored, so it is pushed again rather than failing every retry.
        let hashes: Vec<PathHash> = stored.iter().map(|(_, i)| i.path_hash()).collect();
        let copyable = snapshot.copyable(&hashes).await?;
        for (path, info) in stored {
            let hash = info.path_hash();
            if !copyable.contains(&hash) {
                to_push.push((path, info));
                continue;
            }
        ...
        let pack = async {
            ...
            'pack: while let Some(joined) = compressed.next().await {
                ...
                for frame in frames {
                    ...
                    if builder.compressed_size() >= self.pack_target_size || builder.is_full() {
        ...
        let consumer = async {
            ...
            pack_stream
                .map(|mut pack| async move {
                    upload_pack(&self.backend, &pack).await?;
                    ...
                    Ok::<_, Error>((size, pack))
                })
                .buffer_unordered(UPLOAD_CONCURRENCY)
                .try_collect::<Vec<(u64, chunker::Pack)>>()
                .await
        };
        ...
        if let Some(publish) = self.publish.as_ref().filter(|_| own) {
