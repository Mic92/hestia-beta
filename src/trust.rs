...
pub enum Error {
    ...
    Sign(String),
    /// The verifier could not run. Distinct from a rejection: the head may
    /// well be genuine, so a caller about to delete must stop.
    #[error("{root}: cannot run verifier: {source}")]
    Verify {
        root: String,
        #[source]
        source: std::io::Error,
...
fn glob(pattern: &str, s: &str) -> bool {
    match pattern.split_once('*') {
        None => pattern == s,
        Some((head, tail)) => s.strip_prefix(head).is_some_and(|rest| {
            rest.char_indices()
                .map(|(i, _)| i)
                .chain([rest.len()])
...
impl Trust {
    ...
    /// `Ok(false)`: no row accepts the proof. `Err`: a verifier could not
    /// be run at all, which says nothing about the head.
    pub async fn verify(&self, root: &str, payload: &[u8], bundle: &[u8]) -> Result<bool, Error> {
        if self.is_open() {
            return Ok(true);
        }
        ...
        let io = |source| Error::Verify {
            root: root.to_owned(),
            source,
        };
        let dir = tempfile::tempdir().map_err(io)?;
        let [blob, file] = ["blob", "bundle.json"].map(|f| dir.path().join(f));
        tokio::fs::write(&blob, payload).await.map_err(io)?;
        tokio::fs::write(&file, bundle).await.map_err(io)?;
...
mod tests {
    ...
    fn globs() {
        ...
        assert!(glob("pr-*-é", "pr-日本-é") && !glob("a*b", "aé"));
    }
