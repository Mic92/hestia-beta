...
async fn keys_map_to_prefixed_objects() {
    timed(async {
        ...
        assert!(
            matches!(b.put(&pack_key, pack.clone()).await, Err(Error::Exists(_))),
            "keys are create-only"
        );
        ...
        assert_eq!(listed(&b, "g-").await, Vec::<String>::new());
...
async fn gc_over_s3_with_lagging_listing() {
    timed(async {
        ...
        assert_eq!((stats.roots, stats.deleted), (1, 1), "{stats:?}");
        ...
        sim.push("main", &[], &[&a, &b]).await;
        sim.run_gc(policy.clone(), T0 + 60 * HOUR).await;
        assert_eq!(
            (count("pack"), count("seg")),
            (2, 3),
            "new base plus the loaded view (previous base, re-claim) for one epoch"
        );
        fake.set_clock(T0 + 62 * HOUR);
        sim.run_gc(policy, T0 + 62 * HOUR).await;
        assert_eq!(
            (count("pack"), count("seg")),
            (2, 1),
            "quiet root converges"
        );
        sim.assert_readable(&[&a, &b]).await;
