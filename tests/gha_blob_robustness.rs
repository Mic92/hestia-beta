...
#[tokio::test]
async fn exact_range_responses_still_work() {
    tokio::time::timeout(TEST_TIMEOUT, async {
        // Sanity check against a well-behaved server (axum's stub above is
        // intentionally broken, so spin a correct one here).
        async fn correct(headers: HeaderMap) -> impl IntoResponse {
            match headers.get(header::RANGE).and_then(|v| v.to_str().ok()) {
                ...
                Some(value) => {
                    ...
                    (
                        ...
                        BLOB[start..=end.min(BLOB.len() - 1)].to_vec(),
                    )
                }
            }
        }
        let router = Router::new().route("/blob", get(correct));
        ...
        let chunk = blob::get(&http, &url, Some(100..200)).await.unwrap();
        assert_eq!(chunk.as_ref(), &BLOB[100..200]);

        // 1-byte LRU touch.
        let touch = blob::get(&http, &url, Some(0..1)).await.unwrap();
        assert_eq!(touch.as_ref(), &BLOB[0..1]);

        // Tail range up to the last byte.
        let tail = blob::get(&http, &url, Some(990..1000)).await.unwrap();
        assert_eq!(tail.as_ref(), &BLOB[990..1000]);
    })
    .await
    .expect("test timed out");
}
