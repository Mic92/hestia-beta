...
impl RunnerMap {
    pub fn new(overrides: &[String]) -> Result<Self, Error> {
        let mut map: BTreeMap<String, Vec<String>> = DEFAULT_RUNNERS
            .iter()
    ...
    fn get(&self, system: &str) -> Option<&Vec<String>> {
        self.map.get(system)
    }
}
...
async fn run_inner(args: &MatrixArgs) -> Result<(), Error> {
    ...
    let runners = RunnerMap::new(&args.runners)?;
    let rows = build_rows(
        &jobs,
        &runners,
...
mod tests {
    ...
    fn runner_map_defaults_overrides_and_meta_os() {
        let runners = RunnerMap::new(&["x86_64-linux=self-hosted, big".to_string()]).unwrap();
        ...
        assert!(matches!(
            RunnerMap::new(&["nonsense".to_string()]),
            Err(Error::RunnerSpec(_))
        ));
        assert!(matches!(
            RunnerMap::new(&["riscv64-linux=".to_string()]),
            Err(Error::RunnerSpec(_))
        ));
    }
