...
`nix flake check` builds everything sequentially on one runner, and the
usual matrix workaround makes every job re-evaluate the flake.
`Mic92/hestia/matrix` does the evaluation once: an eval job runs
`nix-eval-jobs`, uploads each check's `.drv` closure to the cache, and
outputs a matrix of the checks that still need building. Every matrix job
then builds its derivation directly — no second evaluation, no flake
plumbing, and already-cached checks get no job at all. The output is
compatible with [nix-github-actions], so existing workflows switch by
swapping the eval step.
...
* Each matrix row also carries `attr`, so `nix build .#${{ matrix.attr }}`
  works as a per-job-eval fallback (the migration path from
  nix-github-actions).
